...
from mcp_connection import get_mcp_tools
...
def extract_crew_result_text(result: Any) -> str:
    """
    Extract the best final answer from CrewAI output.
    Supports CrewOutput objects, plain strings, and JSON tool-call responses.
    """

    if result is None:
        return ""

    # Try common CrewAI attributes
    for attr in ("raw", "final_output", "output"):
        ...
        if value:
            ...
            # Try to parse JSON tool-call output
            try:
                obj = json.loads(text)

                if isinstance(obj, dict):
                    if (
                        obj.get("name") == "answer_user"
                        and "parameters" in obj
                        and "value" in obj["parameters"]
                    ):
...
def run_delegation_crew(
    ...
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
    mcp_adapter, mcp_tools = get_mcp_tools()
    ...
    "Supervisor is preparing local function tools before delegation.",
    "Supervisor Agent",
)
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
    ...
    "executing",
    "Calling MCP Tool",
    "Specialist agents may invoke analytics MCP tools if required.",
    "System",
)
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
    ...
    add_event(
        ...
        "Function tools and MCP tools returned their results successfully.",
        "System",
)
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
...
with st.sidebar:
    ...
    for agent, tools in function_tools.items():
...
if user_prompt:
# ---------------------------------------------------------
# Add uploaded dataset information to the prompt
# ---------------------------------------------------------
...
Important:
- The uploaded CSV is the dataset to analyze.
- Use this dataset for all analysis.
- If dataset analysis is required, delegate to the Data Analyst Agent.
- If machine learning recommendations are required, delegate to the Data Scientist Agent.
- Use the appropriate available function tools and MCP tools whenever needed.
